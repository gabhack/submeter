...
class UserController extends Controller
{
    ...
    function ConfigurarIntervaloNavegacion(Request $request)
    {
        ...
        switch ($request->label_intervalo) {
            ...
            case 'Año Actual':
                ...
                break;
            case 'Última Hora':
                $date_from = \Carbon\Carbon::now()->subHour()->toDateTimeString();
                $date_to = \Carbon\Carbon::now()->toDateTimeString();
                $interval = 12; // Un nuevo identificador de intervalo para 'Última Hora'
    ...
    function InformesPeriodicosAlertas($id, Request $request)
    {
        ...
        if (($id != 0 && Auth::user()->id == $id) || Auth::user()->tipo == 1) {
            ...
            return view('informes_periodicos_alertas.informes_periodicos_alertas', compact(
                'user',
                'titulo',
                'cliente',
                'id',
                ...
                'informes_analizadores_programados',
                'alertas_programadas',
                'tipo_count',
                'contador_label',
                'dir_image_count',
        ...
        if ($user->tipo == 1) {
                ...
                if (!empty($contador)) {
                    ...
                    if ($db !== null) {
                        ...
                        $db->table("Area_Cliente")->where("ID", $data_area->ID)
                            ->update([
                                "TIPO_DE_CONTRATO_ENERGIA" => \DB::raw(intval($request['tipo_contrato_' . $i])),
            ...
            if ($request['cantidad_new_cont'] > 0) {
                for ($i = count($cont); $i < ($request['cantidad_new_cont']); $i++) {
                    ...
                    if ($db !== null) {
                        $db->table("Area_Cliente")->update([
                            "TIPO_DE_CONTRATO_ENERGIA" => $request['tipo_contrato_' . $i],
    ...
    function areaCliente($id, Request $request)
    {
        ...
        if (($id != 0 && Auth::user()->id == $id) || Auth::user()->tipo == 1) {
            ...
            if ($tipo_count < 3) {
                return view('User.area_cliente', compact(
                    'user',
                    'potencia_contratada',
                    'precio_energia',
                    'precio_potencia',
                    'alquiler_equipo_medida',
                    'impuesto',
                    'reduccion',
                    'factor',
                    'ctrl',
                ...
                return view('Gas.area_cliente', compact(
                    'user',
                    'ctrl',
                    'tipo_count',
                    'contador_label',
    ...
    function storeAreaCliente(Request $request, $id)
    {
        ...
        if (count($dias_linea) > 0 && count($potencias_linea) > 0) {
            foreach ($dias_linea as $index => $dia) {
                ...
                $db->table('Linea_Base')->where('DiaDeSemana', $dia)->update([
                    'EAct imp(kWh)' => $potencias_linea[$index],
                    'date_start' => $fecha_inicio_linea_base,
                    "date_end" => $fecha_fin_linea_base
                ]);
            }
        }
    ...
    function AnalizadoresGraficas($user_id, $group_id, $id, Request $request)
    {
        ...
        if (($id != 0 && Auth::user()->id == $id) || Auth::user()->tipo == 1) {
            ...
            return view('analizadores.analizador_graficas', compact(
                ...
                'analizador',
                'total_energias',
                'datos_analizador_potencia',
                'datos_analizador_corriente',
                'total1',
                'total2',
                'total3',
                'total4',
