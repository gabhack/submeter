...
@section('intervals')
    <div class="content intervals">
        <div class="intervalo-izquierda">
            <button type="button" class="btn active btn-middle btn-primary-submeter" data-toggle="modal"
                data-target="#myModal">
                <i class="fa fa-plus"></i>
                <div class="d-sm-inline">Intervalos de Períodos ({{ $label_intervalo }})</div>
                <div class="d-none d-sm-none">{{ $label_intervalo }}</div>
            </button>
        </div>
        <div
            class="intervalo-derecha col-8 col-lg-6 text-center text-lg-right mt-1 mt-sm-3 mt-lg-1 pr-lg-5 font-nav-buttons">
            <form id="form_navigation" action="{{ route('config.navigation') }}" method="POST">
                ...
                @if (isset(Session::get('_flash')['current_date']))
                    <button type="button" class="btn active btn-link btn-auto-responsive"><i class="fas fa-pencil-alt"
                            style="background-color: #286090; padding: 0.2rem; border-radius: 0.3rem; margin-right: 0.3rem;"
                            aria-hidden="true"></i> {{ Session::get('_flash')['current_date'] }}</button>
                @else
                    <button type="button" class="btn active btn-link btn-auto-responsive"><i class="fas fa-pencil-alt"
                            style="background-color: #286090; padding: 0.2rem; border-radius: 0.3rem; margin-right: 0.3rem;"
                            aria-hidden="true"></i> {{ $label_intervalo }} </button>
                @endif
...
@section('counters')
    <div class="content counters">
        <a class="btn btn-sm float-right float-right-custom float-right-custom-1 btn-outline-primary"
            href="{{ route('statistics.list', ['type' => $type, 'user_id' => $user->id]) }}" class=" hvr-bounce-to-right">
            <i class="fa fa-cogs"></i><span>Editar configuración</span>
        </a>
        @if ($type == 'indicadores')
            <a class="btn btn-sm float-right float-right-custom float-right-custom-2 btn-outline-secondary mr-2"
                href="{{ route('statistics.manual_data', ['user_id' => $user->id]) }}" class=" hvr-bounce-to-right">
        ...
        <ul class="btn-list nav-header nav-pills contador-pill nav-pills-inline">
            @foreach ($user->energy_meters as $i => $contador)
                @if (App\Http\Controllers\GroupsController::checkContadorMenu(
                        $user->id,
                        $type === 'indicadores' ? '16' : '15',
                        $contador->id))
                    @if ($contador->id == $user->current_count->meter_id)
                        <li class="btn active nav-item">
                            <a class="nav-link bg-submeter-3 active text-white"
                                href="{{ route('energymeter.change', ['user_id' => $user->id, 'meter_id' => $contador->id]) }}">
                                <i class="fa fa-clock-o"></i> <span>{{ $contador->count_label }}</span>
                            </a>
                        </li>
                    @else
                        <li class="btn nav-item">
                            <a class="nav-link bg-submeter-5"
                                href="{{ route('energymeter.change', ['user_id' => $user->id, 'meter_id' => $contador->id]) }}">
                                <i class="fa fa-clock-o"></i> <span>{{ $contador->count_label }}</span>
                            </a>
                        </li>
...
@section('content')
            ...
            <div>
                <div class="container" style="width:100%; display: inline-block">
                    <div class="row">
                        ...
                        <div class="col">
                            <img class="float-right" width="60" height="60"
                                src="{{ asset('images/Logo_WEB_Submeter.png') }}">
                        </div>
                    </div>
                </div>
                <div class="col-12">
                        ...
                        <div class="card-row">
                            <div class="input-group d-flex py-0">
                                <div class="input-group-prepend">
                                    <span
                                        class="input-group-text bg-transparent font-weight-bold border-0">Intervalo:</span>
                                </div>
                                <label for="staticEmail" class="col-12 col-lg-8 col-form-label d-lg-inline">
                                    <h6>Desde {{ $date_from }} hasta {{ $date_to }}</h6>
                                </label>
                            </div>
                        </div>
                    </div>
            ...
            <div class="column column-custom">
                <div class="row">
                    @foreach ($configurations as $index => $config)
                        @php
                            $prevConfig = $index > 0 ? $configurations[$index - 1] : null;
                            $nextConfig = $index < count($configurations) - 1 ? $configurations[$index + 1] : null;
                            $marginStyle = '';

                            if ($config->Size == 50) {
                                if ($prevConfig && $prevConfig->Size == 100) {
                                    $marginStyle = 'margin-right: 1.5%;';
                                } elseif (!$prevConfig && $nextConfig && $nextConfig->Size == 50) {
                                    $marginStyle = 'margin-right: 1.5%;';
                                } elseif ($prevConfig && $prevConfig->Size == 50) {
                                    $marginStyle = 'margin-left: 1.5%;';
                                }
                            }
                        @endphp
                        <div data-chart-indicator="{{ $config->id }}" data-type="{{ $type }}"
                            data-counter-name="{{ $contador2->count_label }}"
                            data-chart-indicator-name="{{ $config->name }}"
                            style="width: {{ $config->Size == 50 ? '48.5%' : '100%' }}; {{ $marginStyle }}">
                        </div>
                    @endforeach
                </div>
            </div>
        @else
            <div class="column column-custom">
                @foreach ($configurations as $config)
                    <div data-chart-indicator="{{ $config->id }}" data-type="{{ $type }}"
                        data-counter-name="{{ $contador2->count_label }}"
                        data-chart-indicator-name="{{ $config->name }}" class="col-12">
                    </div>
                @endforeach
            </div>
        @endif
    ...
    <div class="content-main content-main-responsive col-md-12 pl-2 pr-4 content-90 gray-bg">
        ...
        <form method="post" id="form-pdf"
            action="{{ route('exportacion.pdf', ['titulo' => $titulo, 'date_from' => $date_from, 'date_to' => $date_to, 'contador_label' => $contador->count_label]) }}">
            {{ csrf_field() }}
        </form>
    </div>
@endsection
...
@section('scripts')
    ...
    <script>
        ...
        function createPlots() {
            ...
            for (var iK = 0; iK < plots.length; iK++) {
                ...
                for (var i = 0; i < seriesValues.length; i++) {
                    ...
                    var conf = {
                        cursor: "zoom-in",
                        type: plot[iK].chart_type,
                        showInLegend: plot[iK].chart_type != 'pie' ? true : false,
                        visible: true,
                        bevelEnabled: true,
                        markerSize: 0,
                ...
                var chart = new CanvasJS.Chart(cntPlot.attr("id"), {
                    ...
                    axisX: {
                        ...
                        interval: dataPlot[0].dataPoints.length > 24 ? 1 :
                        1, //# @Leo W* vamos asegurarnos de que cuando los intervaloes sean mas de 2 puntos no se encimen en el eje de las X
                        tickColor: "#004165"
                    },
                    axisY: {
                        lineColor: "#004165",
        ...
        var exportProdPdf = function(objContainer) {
            ...
            var input = $("<input name='elements[]' type='hidden' value='" + btoa(unescape(encodeURIComponent(
                header))) + "' />");
